...
""", file=f)
    ...
    print("\t{", file=f)
    print("#ifdef TIME_PBD_SIGNAL_EMISSION", file=f)
    print("\t\tint64_t clock[3];", file=f)
    print("\t\tclock[0] = g_get_monotonic_time();", file=f)
    print("#endif", file=f)
    ...
    print("", file=f)
    print("#ifdef TIME_PBD_SIGNAL_EMISSION", file=f)
    print("\t\tclock[1] = g_get_monotonic_time();", file=f)
    print("#endif", file=f)
    print("", file=f)
    if not v:
    ...
    print("", file=f)
    print("#ifdef TIME_PBD_SIGNAL_EMISSION", file=f)
    print("\t\tclock[2] = g_get_monotonic_time();", file=f)
    print("\t\tconst int64_t elapsed_time = clock[2] - clock[0];", file=f)
    print("\t\tif (elapsed_time > TIME_PBD_SIGNAL_EMISSION) {", file=f)
    print("\t\t\tprintf(\"signal () took %.3f + %.3f ms\\n\", (clock[1] - clock[0]) / 1000.f, (clock[2] - clock[1]) / 1000.f);", file=f)
    print("\t\t\tPBD::stacktrace (std::cerr, 14);", file=f)
    print("\t\t}", file=f)
    print("#endif", file=f)
    print("", file=f)
    if not v:
